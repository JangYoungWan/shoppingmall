...
public class CartController {
    ...
    //장바구니 상품의 수량을 업데이트하는 요청을 처리할 수 있도록 하는 로직
    //HTTP 메소드에서 PATCH는 요청된 요청된 자원의 일부를 업데이트 할때 PATCH를 사용한다. 장바구니 상품의 수량만 업데이트 하기 때문에 @PatchMapping을 사용
    @PatchMapping(value = "/cartItem/{cartItemId}")
    public @ResponseBody ResponseEntity updateCartItem(@PathVariable("cartItemId") Long cartItemId, int count, Principal principal){

        //장바구니에 담겨 있는 상품의 개수를 0개 이하로 업데이트 요청을 할 때 에러메세지를 담아서 반환 한다.
        if (count <= 0){
    ...
    public @ResponseBody ResponseEntity orderCartItem(@RequestBody CartOrderDto cartOrderDto, Principal principal){
        ...
        //생성된 주문 번호와 요청이 성공 했다는 HTTP응답 상태 코드를 반환한다.
