...
public class CartService {
    ...
    public List<CartDetailDto> getCartList(String email){
        ...
        //현재 로그인한 회원의 장바구니 엔티티를 조회한다.
        Cart cart = cartRepository.findByMemberId(member.getId());
        //장바구니 상품을 한번도 안담았을 경우 장바구니 엔티티가 없으므로 빈리스트를 반환한다.
        if (cart == null){
    ...
    //자바스크립트 코드에서 업데이트할 장바구니 상품번호는 조작이 가능하므로 현재 로그인한 회원과 해당 장바구니 상품을 저장한 회원이 같은지 검사하는 로직도 추가
    @Transactional(readOnly = true)
    public boolean validateCartItem(Long cartItemId, String email){
        //현재 로그인한 회원을 조회한다.
        Member curMember = memberRepository.findByEmail(email);
        CartItem cartItem = cartItemRepository.findById(cartItemId).orElseThrow(EntityNotFoundException::new);
        //장바구니 상품을 저장할 회원을 조회한다.
    ...
    //주문 로직으로 전달할 orderDto 리스트 생성, 주문 로직 호출, 주문한 상품은 장바구니에서 제거하는 로직
    public Long orderCartItem(List<CartOrderDto> cartOrderDtoList, String email) {

        List<OrderDto> orderDtoList = new ArrayList<>();

        //장바구니 페이지에서 전달받은 줌누 상품 번호를 이용하여 주문 로직으로 전달할 orderDto 객체를 만든다.
        ...
        //장바구니에 담은 상품을 주문하도록 주문 호직을 호출한다.
